...
function cargarPedidosBar() {
    fetch("api/obtener_pedidos_bar.php")
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            console.log("Pedidos de bar recibidos:", data);
            let contenedor = document.getElementById("pedidos-bar-container");
            if (!contenedor) return;
            contenedor.innerHTML = ""; // Limpiar el contenedor para evitar duplicados

            if (data.length === 0) {
                contenedor.innerHTML = "<p class='text-center text-muted'>No hay pedidos de bar pendientes.</p>";
                return;
            }
                ...
                card.innerHTML = `
                    <div class="card">
                        <div class="card-header bg-info text-white"
                             data-id="${pedido.id}" 
                             onclick="moverPedidoBar(${pedido.id}, this)">
                            <h5>Mesa ${pedido.num_mesa}</h5>
                        </div>
                        <div class="card-body">
                            <p><strong>Bebida:</strong> ${pedido.nombre}</p>
        ...
        .catch(error => console.error("Error al cargar pedidos de bar:", error));
}
