...
$base = dirname(__DIR__); // …/Proyecto_IC25-ADI/src
...
function enviarCorreoBienvenida($correoDestino, $nombreUsuario, $contrasenaTemporal)
{
    ...
    try {
        ...
        $mail->SMTPAuth    = true;
        $mail->Username    = 'kendallchacon999@gmail.com';
        $mail->Password    = 'objcxqkcnsqenmcp';
        $mail->SMTPSecure  = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port        = 587;

        // Remitente
        $mail->setFrom('kendallchacon999@gmail.com', 'Restaurante RSI');
        $mail->addAddress($correoDestino, $nombreUsuario);

        // Incrustar imágenes (CID)
        global $base;
        $logoPath    = dirname($base) . '/assets/img/logo.png';      // ajusta ruta si es necesario
        $perfilPath  = dirname($base) . '/assets/img/logo.png';  // o tu imagen de perfil
        $mail->addEmbeddedImage($logoPath,   'logo_cid');
        $mail->addEmbeddedImage($perfilPath, 'perfil_cid');

        // Contenido
        $mail->isHTML(true);
        $mail->Subject = 'Bienvenido a Restaurante RSI';
        $mail->Body    = '
                ...
                <div style="max-width:600px;background:#ffffff;border-radius:10px;box-shadow:0 4px 10px rgba(0,0,0,0.1);overflow:hidden">
                  ...
                  <div style="padding:30px;text-align:center;color:#333">
                    ...
                    <p style="font-size:16px;line-height:1.5;margin:0 0 20px">
                      Te damos la bienvenida al sistema de administración del <strong>Restaurante RSI</strong>.
                    </p>
                    ...
                    <!-- Botón de Acción -->
                    <a href="https://tu-dominio.com/perfil" 
